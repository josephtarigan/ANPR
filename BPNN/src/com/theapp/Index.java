...
public class Index extends JFrame {
	 ...
	 */
	private static AlphaNumericNeuralNetworks alphaNumericNeuralNetworks = new AlphaNumericNeuralNetworks();
	/*
	 * Looping control
	 */
	 ...
	 * Thread for training
	 */
	private static Thread trainingThread, wc, updateData, uc, rd;
	
	/*
	 * Execution time 
	 */
	@SuppressWarnings("unused")
	private long startTime, endTime, elapsedTime = 0, tempTime, currentTime;
	/*
	 * JFreeChart objects
	 */
	private static XYSeries trainingSeries = null;
	private static XYSeriesCollection trainingSeriesCollection = null;
    private static JFreeChart trainingChart = null;
    private static ChartPanel chartPanel = null;
    /*
     * XYSeries holder
     */
    private  static List<Double> MSESeries = new ArrayList<Double>();
    private static List<Integer> epochSeries = new ArrayList<Integer>();
	...
	public Index() throws IOException {
		...
		elapsedTimeValue = new JLabel("0", SwingConstants.LEFT);
		...
		//chart
		chartPanel = new ChartPanel(trainingChart);
			...
			while(trainingLoop && MSE > targetError){
				...
				for(int i=0; i<CharConfig.CHARSET_COUNT; i++){
					...
					try {
						...
						/*
						 * Thread for updating UI data
						 */
						updateData = new UpdateTrainingData(MSE, epoch, startTime - System.currentTimeMillis());
						updateData.start();
						
						/*
						 * Thread for refreshing data series for the chart
						 */
						rd = new RefreshData();
						rd.start();
	...
	class UpdateTrainingData extends Thread{
		...
		public UpdateTrainingData(double MSE, int epoch, long time){
			...
			final long finalTime = Math.abs(time);
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					//minute
					minute = (finalTime/(1000 * 60)) % 60;
					
					//hour
					hour = (finalTime/(1000 * 60 * 60));
	...
	/*
	 * Refresh series of data for JFreeChart object
	 */
